...
@serve.deployment(
    route_prefix="/image_predict", num_replicas=6, ray_actor_options={"num_gpus": 1}
)
class ImageModel:
    def __init__(self, modelfname):
        print(
            f"cuda:{ray.get_gpu_ids()[0]}"
        )  # select the gpu on which the DL model will reside
        self.device = torch.device("cuda")
        checkpoint = torch.load(
            modelfname, map_location=lambda storage, loc: storage
    ...
    async def __call__(self, starlette_request):
        ...
        npad0 = int(
            np.ceil(io_shape_wpad[0] / patch_size) * patch_size - io_shape_wpad[0]
        )
        npad1 = int(
            np.ceil(io_shape_wpad[1] / patch_size) * patch_size - io_shape_wpad[1]
        )

        io = np.pad(io, [(0, npad0), (0, npad1), (0, 0)], mode="constant")

        arr_out = sklearn.feature_extraction.image._extract_patches(
            io, (patch_size, patch_size, 3), stride_size
        )
        ...
        for batch_arr in self.divide_batch(arr_out, batch_size):
            batch_index += 1
            print(
                f"PROGRESS: Generating Prediction - Batch {batch_size*batch_index}/{arr_out.shape[0]}",
                flush=True,
            )
            arr_out_gpu = (
                torch.from_numpy(batch_arr.transpose(0, 3, 1, 2) / 255)
        ...
        output = output.reshape(
            arr_out_shape[0], arr_out_shape[1], patch_size, patch_size, output.shape[3]
        )
        ...
        output = output[
            0 : io_shape_orig[0], 0 : io_shape_orig[1], :
...
imagedata = Image.open(r"/path/image")
